...
| 批次上傳 | 拖曳多個 `.ts` 檔案到頁面任何位置，依檔名順序自動建立片段 |
...
拖曳多個 `.ts` 檔案到頁面任何位置（不限片段設定區塊），會依檔名自然排序（如 `1.ts, 2.ts, 10.ts`）自動逐一建立片段並載入檔案，名稱依序遞增（A、B、C...）。

- 拖曳中頁面會顯示提示遮罩
- 也可點擊「片段設定」上方的批次拖放區改用檔案選擇視窗（可多選）
- 播放中無法批次上傳
